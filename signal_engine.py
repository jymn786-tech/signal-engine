...
import time as pytime  # for retries/backoff without shadowing datetime.time
...
def ensure_master_contracts(exchanges):
    """Ensure contract masters are downloaded on fresh CI runners.
    pya3 caches locally on first call; CI often starts empty, so force it here.
    """
    for ex in exchanges:
        for attempt in range(1, 4):
            try:
                alice.get_contract_master(ex)
                print(f"📥 Contract master loaded: {ex}")
                break
            except Exception as e:
                print(f"⚠️ Contract master load failed for {ex} (try {attempt}/3): {e}")
                if attempt < 3:
                    pytime.sleep(2 * attempt)  # simple backoff
                else:
                    # Don't hard-fail; we'll still try search fallback later
                    pass


def instrument_for_symbol(sym, exchange="NSE"):
    """Robust instrument fetch that tolerates missing masters on CI.
    Tries get_instrument_by_symbol first, then falls back to search_instruments.
    """
    try:
        ...
        try:
            ...
            if matches:
                if getattr(matches[0], "symbol", None) != sym:
                    print(f"🔎 Fallback matched {getattr(matches[0], 'symbol', 'UNKNOWN')} for {sym} in {exchange}")
                return matches[0]
        except Exception as se:
            print(f"❌ search_instruments failed for {exchange}:{sym}: {se}")
        raise RuntimeError(f"Instrument not found for {exchange}:{sym}; ensure contract masters are available.") from e
